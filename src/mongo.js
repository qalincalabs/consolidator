...
export async function consolidateCollection(
    ...
    console.log(EJSON.stringify(rule))
  ...
  export async function findAndConsolidate(
    ...
    for (const newItem of newItems) {
      ...
      const itemAmongstConsolidatedOnes = findCollectionItemByItemIds(consolidatedItems, newItem)
      
      let currentItem = {}
  
      if (itemAmongstConsolidatedOnes != null){
        toUpdateItems.splice(toUpdateItems.indexOf(itemAmongstConsolidatedOnes),1)
        consolidatedItems.splice(consolidatedItems.indexOf(itemAmongstConsolidatedOnes),1)
        alreadyUpdatedItems.splice(alreadyUpdatedItems.indexOf(itemAmongstConsolidatedOnes),1)
      ...
      console.log(EJSON.stringify(newItem?.totalPrice))
      console.log(EJSON.stringify(currentItem?.totalPrice))
      ...
      const sameAsCurr = myConsolidator.isSameAsConsolidation(consolidationProfile)
      console.log(sameAsCurr)
      ...
      if (sameAsCurr == false) {
        ...
        console.log("already updated update")
      }
    }
    
    console.log(EJSON.stringify(toUpdateItems,null,2))
    ...
    for (const toUpdateItem of toUpdateItems) {
      ...
      if (toUpdateItem._id == null) {
        ...
        if (toUpdateItem.createdAt == null) {
          toUpdateItem.createdAt = new Date( parseInt( toUpdateItem._id.toString().substring(0,8), 16 ) * 1000 );// TODO not working toUpdateItem._id.getTimestamp() in Atlas
        }
